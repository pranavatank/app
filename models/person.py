...
def add_person(full_name: str, date_of_birth: str = None,
               pan_number: str = None, contact_notes: str = None) -> int:
    """Insert a new person. Returns the new person_id."""
    conn = get_connection()
    cur = conn.execute("""
        INSERT INTO Person (full_name, date_of_birth, pan_number, contact_notes)
        VALUES (?, ?, ?, ?)
    """, (full_name, date_of_birth, pan_number, contact_notes))
    conn.commit()
...
def update_person(person_id: int, full_name: str, date_of_birth: str = None,
                  pan_number: str = None, contact_notes: str = None) -> None:
    """Update person details."""
    conn = get_connection()
    conn.execute("""
        UPDATE Person
        SET full_name = ?, date_of_birth = ?, pan_number = ?, contact_notes = ?
        WHERE person_id = ?
    """, (full_name, date_of_birth, pan_number, contact_notes, person_id))
    conn.commit()
    conn.close()
