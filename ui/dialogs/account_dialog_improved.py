"""
ui/dialogs/account_dialog_improved.py — Improved bank account dialog with bank master and account holder name
"""

from PyQt6.QtWidgets import (
    QDialog, QVBoxLayout, QHBoxLayout, QLabel, QPushButton,
    QLineEdit, QFormLayout, QMessageBox, QComboBox, QDoubleSpinBox,
...
from models.bank import get_all_banks, get_or_create_bank
from models.bank_account import add_account, update_account


class AccountDialog(QDialog):
    """Improved dialog for adding/editing a bank account."""
    ...
    def _create_header(self) -> QFrame:
        ...
        title = QLabel("🏦 " + ("Edit Account" if self.account_data else "Add New Account"))
    ...
    def _create_basic_tab(self) -> QWidget:
        ...
        # Bank Name - Combo with editable option
        ...
        banks = get_all_banks()
        for bank in banks:
            self.bank_combo.addItem(bank["bank_name"], userData=bank["bank_id"])
        
        self.bank_combo.setCurrentIndex(0)
        bank_layout.addWidget(self.bank_combo)
        
        layout.addRow("Bank Name *:", bank_layout)
    ...
    def _load_data(self):
        ...
        # Set bank name in combo
        bank_name = self.account_data.get("bank_name", "")
        idx = self.bank_combo.findText(bank_name)
        if idx >= 0:
            self.bank_combo.setCurrentIndex(idx)
        else:
            self.bank_combo.setEditText(bank_name)

        self.holder_name_input.setText(self.account_data.get("account_holder_name") or "")
    ...
    def _on_save(self):
        ...
        if not bank_name or bank_name == "-- Select or Type New Bank --":
            QMessageBox.warning(self, "Missing Field", "Please select or enter a bank name.")
            return

        # Create/get bank in master table
